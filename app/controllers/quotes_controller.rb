...
# have a before action to make all things, in before action do teh render. Then no double render.
# can assume that by the time it makes the quote, we can assume instance varibales are created.
...
class QuotesController < ApplicationController
  ...
  def make_ups_quote
    ...
    response = ups.find_rates(make_origin, make_destination, make_package)
  ...
  def make_fedex_quote
    ...
    response = fedex.find_rates(make_origin, make_destination, make_package)
    response_rates = response.rates.sort_by(&:price).collect {|rate| [rate.service_name, rate.price]}
    render json: response_rates
  end
